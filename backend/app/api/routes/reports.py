...
def _get_owned_scan_and_project(db: Session, scan_id: uuid.UUID, user: User):
    scan = scan_service.get_scan(db, scan_id)
    if scan is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Scan not found")
    project = project_service.get_project(db, scan.project_id, user_id=user.id)
    if project is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Scan not found")
    return scan, project


@router.post("/scans/{scan_id}/reports", response_model=ReportRead, status_code=status.HTTP_201_CREATED)
def generate_report(
    scan_id: uuid.UUID, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
) -> ReportRead:
    scan, project = _get_owned_scan_and_project(db, scan_id, current_user)
    ...
    file_path = save_report(db, scan, project)
    report = Report(scan_id=scan.id, report_type="html", file_path=str(file_path))
    db.add(report)
    db.commit()
    db.refresh(report)
...
@router.get("/reports/scan/{scan_id}", response_model=list[ReportRead])
def list_reports_for_scan(
    scan_id: uuid.UUID, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
) -> list[ReportRead]:
    _get_owned_scan_and_project(db, scan_id, current_user)
...
def get_report(
    ...
    _get_owned_scan_and_project(db, report.scan_id, current_user)
    return ReportRead.model_validate(report)
...
def download_report(
    ...
    if report is None or not report.file_path:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Report not found")
    _get_owned_scan_and_project(db, report.scan_id, current_user)
    return FileResponse(report.file_path, media_type="text/html", filename=f"codeshieldai-report-{report.scan_id}.html")
