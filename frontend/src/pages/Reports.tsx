...
import { downloadReportUrl, generateReport, listProjects, listScans, listReportsForScan } from "../services/api";
...
export default function Reports() {
  ...
  const isLoading = !scansLoaded || reportQueries.some((q) => q.isLoading);
  const allReports = reportQueries.flatMap((q) => q.data ?? []);
  const scansWithoutReport = completedScans.filter(
    (scan) => !allReports.some((r) => r.scan_id === scan.id)
  );

  const generateMutation = useMutation({
    mutationFn: (scanId: string) => generateReport(scanId),
    onSuccess: (_, scanId) => {
      queryClient.invalidateQueries({ queryKey: ["reports", scanId] });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Generated HTML security reports for completed scans.</p>
      </div>

      {!isLoading && scansWithoutReport.length > 0 && (
        <div className="card p-5">
          <h3 className="text-sm font-medium text-slate-300 mb-3">Completed scans without a report yet</h3>
          <div className="divide-y divide-border">
            {scansWithoutReport.map((scan) => (
              <div key={scan.id} className="flex items-center justify-between py-2.5">
                <Link to={`/scans/${scan.id}`} className="text-sm text-slate-300 hover:text-primary font-mono">
                  {scan.id.slice(0, 8)}… — {scan.scan_type}
                </Link>
                <button
                  onClick={() => generateMutation.mutate(scan.id)}
            ...
            {allReports.map((r) => (
              <div key={r.id} className="flex items-center justify-between py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <Link to={`/scans/${r.scan_id}`} className="text-sm text-slate-200 hover:text-primary uppercase">
